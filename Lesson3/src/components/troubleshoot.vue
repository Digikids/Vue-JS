  ...
  <div class="calculator">
    ...
    <div class="field">
      ...
      <div class="tip-button">
        <button v-for="pct in [10,15,20,25]"    
        :key="pct"
    ...
    <div class="results">
      ...
      <div class="results-row total">
        ...
        <span>KSh {{ totalPerPerson.toFixed }}</span>
      </div>
    </div>
  </div>
</template>
...
.tip.button{ display: flex; gap:8px}
.tip-button button {
  flex:1; padding:10px; border: 2px solid #ddd;
  border-radius:8px; background:white;cursor:pointer;font-size:15px;
}

.tip-button button.active {background-color: #42b883; color:white; border-color:#42b883}
.results {background: #3549; border-radius:10px;padding:20px;margin-top: 24px;}
.results-row {display:flex;justify-content: space-between;color: white;padding: 8px 0;
font-size:16px;}
.results-row.total {font-size:20px;font-weight:bold;border-top:1px solid rgba(255,255,255,0.2);
padding-top:14px;}
</style>